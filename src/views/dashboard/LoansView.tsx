...
import { getAdminReservations, cancelReservation } from '../../api/reservations';
import {
  BookOpenIcon,
  MagnifyingGlassIcon,
  FunnelIcon,
...
import LoanInfoModal from '../../components/library/LoanInfoModal';
...
export default function LoansView() {
  ...
  const [loanInfoUserId, setLoanInfoUserId] = useState<string | null>(null);
  const [loanInfoUserName, setLoanInfoUserName] = useState<string | undefined>();
  ...
  const loanColumns = [
    ...
    },
    {
      ...
      render: (loan: Loan) => {
        ...
        return (
          <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${loan.status === 'RETURNED'
    ...
    },
    {
      ...
      render: (res: Reservation) => (
        <div className="flex flex-col gap-1">
          <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest border w-fit ${res.requestedLoanType === 'HOME'
            ? 'bg-blue-500/10 text-blue-400 border-blue-500/20'
  ...
  return (
    <div className="space-y-8">
      ...
      <LoanInfoModal
        isOpen={!!loanInfoUserId}
        onClose={() => { setLoanInfoUserId(null); setLoanInfoUserName(undefined); }}
        userId={loanInfoUserId || ''}
        userName={loanInfoUserName}
      />
      ...
      <div className="flex p-1 bg-slate-900/50 border border-slate-800 rounded-2xl w-fit">
        <button
          ...
          className={`flex items-center gap-2 px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'LOANS' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'
            }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'RESERVATIONS' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'
            }`}
        >
      ...
      <div className="space-y-6">
        <div className="bg-slate-900/40 backdrop-blur-xl border border-slate-800 p-4 rounded-3xl flex flex-col lg:flex-row gap-4 items-center justify-between">
          <div className="relative w-full lg:max-w-md flex items-center gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-slate-800/30 border border-slate-700/50 rounded-xl py-2.5 pl-12 pr-4 text-white focus:outline-hidden focus:ring-2 focus:ring-blue-500/40 transition-all text-sm placeholder:text-slate-500"
                value={searchTerm}
                onChange={(e) => {
            ...
            <button
              ...
              className="px-4 py-2.5 bg-blue-600/10 hover:bg-blue-600/25 text-blue-400 border border-blue-500/20 hover:border-blue-500/30 rounded-xl transition-all active:scale-95 flex items-center gap-2 font-bold text-xs shadow-lg shadow-blue-950/20 shrink-0"
          ...
          {activeTab === 'LOANS' && (
            <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto lg:justify-end">
              <div className="flex items-center gap-2 bg-slate-800/30 border border-slate-700/50 rounded-xl px-3.5 py-2.5 text-xs font-bold text-slate-300 focus-within:ring-2 focus-within:ring-blue-500/40 transition-all">
                <FunnelIcon className="w-4 h-4 text-slate-400" />
                <select
                  className="bg-transparent text-white font-extrabold focus:outline-hidden border-none cursor-pointer"
                  value={roleFilter}
                  onChange={(e) => {
                ...
                >
                  ...
                  <option value="STUDENT" className="bg-slate-900">ESTUDIANTES</option>
                  <option value="TEACHER" className="bg-slate-900">PROFESORES</option>
                </select>
              </div>
        ...
        <div className="space-y-6">
          <LibraryTable
            ...
            renderActions={(item: Loan | Reservation) => {
              if (activeTab === 'LOANS') {
                return (
                ...
                const res = item as Reservation;
                if (res.status === 'PENDING') {
                  return (
                    <div className="flex justify-end gap-2">
                      <button
                        className="flex items-center gap-2 px-4 py-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all active:scale-95"
